...
- ✅ **评论删除功能修复**：修复了删除评论时的外键约束错误，实现了级联删除功能，支持删除带有回复的评论
- ✅ **级联删除机制**：完善了评论删除逻辑，删除父评论时会自动递归删除所有子回复，确保数据完整性
- ✅ **评论权限验证**：修复了删除评论时缺少 Authorization 头的问题，确保只有登录用户才能删除评论
- ✅ **JSON 序列化优化**：解决了评论和博客 DTO 中的循环引用问题，将 User 对象替换为 userId 和 userAccount 字段
- ✅ **评论系统优化**：评论和回复现在显示真实用户名而非"匿名用户"，提升用户互动体验
- ✅ **评论提示优化**：移除了误导性的"等待审核"提示，评论提交后直接显示成功信息
- ✅ **用户身份识别**：完善了前后端用户身份传递机制，支持从 token 解析 userId 并关联评论
- ✅ **UI 交互优化**：统一首页导航栏搜索按钮的悬停效果，与其他导航按钮保持一致的视觉反馈
- ✅ **搜索对话框美化**：重新设计搜索对话框的关闭按钮，采用现代化的圆形设计，提供更自然的交互体验
- ✅ **视觉一致性提升**：优化按钮悬停动画效果，确保整个界面的交互反馈统一协调
- ✅ **用户体验改进**：关闭按钮采用渐进式透明度设计，默认状态低调融入界面，悬停时提供清晰的视觉反馈
- ✅ **排序功能修复**：修复了文章排序功能的关键问题，现在支持按发布时间、点赞数、热度（浏览量）正确排序
- ✅ **排序参数优化**：统一排序字段映射，发布时间使用 `created_at`，点赞排序使用 `like_count`，热度排序使用 `view_count`
- ✅ **后端逻辑完善**：修复了无搜索条件时排序失效的问题，确保所有排序方式都能正常工作
- ✅ **用户界面优化**：将管理员和用户后台的"退出登录"按钮改为"返回首页"按钮，提升用户体验
- ✅ **导航功能改进**：优化了后台页面的导航逻辑，用户可以直接返回首页而无需退出登录
- ✅ **布局样式修复**：修复了管理员后台头部布局问题，确保欢迎信息和按钮正确排列
